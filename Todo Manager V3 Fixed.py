...
class TodoManager:
    ...
    def _load_tasks(self):
        try:
            ...
            backup_files = sorted(TODO_FILE.parent.glob(".todo_*.bak"), key=lambda f: f.name.split("_")[1].split(".")[0], reverse=True)
    ...
    def add_task(self, content, priority=None):
        ...
        task = {"id": self.next_id, "content": content.strip(), "priority": priority, "status": "pending", "created": now_str, "modified": now_str}
    ...
    def edit_task(self, task_id, new_content=None, new_priority=None):
        ...
        if new_content is not None:
            ...
            modified = True
        if new_priority:
            if not new_priority.strip() or new_priority not in VALID_PRIORITIES:
                raise ValueError(f"无效优先级，可选: {', '.join(VALID_PRIORITIES)}")
